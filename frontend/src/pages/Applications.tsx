...
const STATUS_STYLES: Record<string, { bg: string; text: string; icon: React.ElementType }> = {
  pending_approval: { bg: "bg-amber-100", text: "text-amber-700", icon: ClockIcon },
  approved:         { bg: "bg-blue-100",  text: "text-blue-700",  icon: CheckCircleIcon },
  submitted:        { bg: "bg-green-100", text: "text-green-700", icon: CheckCircleIcon },
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex gap-2 mb-6">
        {["", "pending_approval", "submitted", "rejected", "failed"].map((s) => (
          <button
            key={s}
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {apps.map((app) => {
            const style = STATUS_STYLES[app.status] ?? STATUS_STYLES["pending_approval"];
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {app.match_score && (
                        <span className="text-xs text-gray-500">
                          Match: <span className="font-semibold text-blue-600">{app.match_score.toFixed(1)}%</span>
                        </span>
                      )}
                    </div>
                    <p className="text-sm text-gray-500 font-mono text-xs">ID: {app.id.slice(0, 16)}…</p>
                    {app.confirmation_number && (
                      <p className="text-sm text-green-600 mt-1">
                        Confirmation: <span className="font-medium">{app.confirmation_number}</span>
                      </p>
                    )}
                    {app.error_message && (
                      <p className="text-sm text-red-500 mt-1">{app.error_message}</p>
                    )}
